...
exports.setTaskStatus = functions.https.onCall(async (data, context) => {
    try{
        ...
        const { projectID, performer: {uid} } = task.data();
...
exports.onProjectDelete = functions.firestore.document('projects/{projectID}').onDelete(async (snap, context) => {
    try{
...
exports.onTaskDelete = functions.firestore.document('tasks/{taskID}').onDelete(async (snap, context) => {
    try{
...
exports.moveMessageFile = functions.https.onCall(async (data, context) => {
    try{
        ...
        if(!members.includes(context.auth.uid)) return Promise.reject('unauthorized');
        const file = bucket.file(`temp/projects/${data.projectID}/messages/${data.fileID}`);

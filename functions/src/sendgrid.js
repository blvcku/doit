...
exports.sendPostMessage = functions.firestore
    ...
    .onCreate(async (snap, context) => {
        try {
            ...
            const {
                message,
                author: { email: senderEmail, displayName: senderName },
            } = snap.data();
...
exports.sendTaskMail = functions.firestore
    .document('tasks/{taskID}')
    .onCreate(async (snap, context) => {
        try {
            const {
                performer: { uid: userID },
                projectID,
            } = snap.data();
...
exports.sendVerificationMail = async (uid) => {
    try {
        const { emailVerified, email, displayName } = await admin
            .auth()
            .getUser(uid);
...
exports.sendPasswordResetMail = functions.https.onCall(
    async (data, context) => {
        try {
            const { email } = data;

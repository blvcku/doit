...
exports.onAnswerCreate = functions.firestore.document('forms/{formID}/answers/{answerID}').onCreate(async (snap, context) => {
    try{
        ...
        for(const [ questionID, { answers, input } ] of Object.entries(answer.answers)){
            const answersSumRef = formsRef.doc(formID).collection('questions').doc(questionID).collection('answersSum').doc('answersSum');
...
exports.onFormDelete = functions.firestore.document('forms/{formID}').onDelete(async (snap, context) => {
    try{

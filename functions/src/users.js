    ...
    try {
        ...
        await formsRef
            ...
            .then((forms) => {
                forms.forEach(
                    async (form) =>
                        await firestore.recursiveDelete(formsRef.doc(form.id)),
                );
            });
        await projectsRef
            .where('authorID', '==', context.auth.uid)
        ...
        await usersRef
            .where('friends', 'array-contains', context.auth.uid)
            .get()
            .then((friends) => {
                friends.forEach(
                    async (friend) =>
                        await usersRef.doc(friend.id).update({
        ...
        await usersRef
            ...
            .then((invites) => {
                invites.forEach(
                    async (invite) =>
                        await usersRef.doc(invite.id).update({
        ...
        await usersRef
            .where('requests', 'array-contains', context.auth.uid)
            .get()
            .then((requests) => {
                requests.forEach(
                    async (request) =>
                        await usersRef.doc(request.id).update({
        ...
        await postsRef
            ...
            .then((posts) => {
                posts.forEach(
                    async (post) =>
                        await firestore.recursiveDelete(postsRef.doc(post.id)),
                );
            });

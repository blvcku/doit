...
import PlusIconWhite from '../../../images/project/tasks/plus.svg';
...
const TaskEditBody = ({steps, setSteps, setTitle, setDescription, setSelectedFile, title, description, selectedFile}) => {
    ...
    return(
        <TaskEditBodyContainer>
            ...
            <Label>
                Description:
                <textarea onChange={handleDescriptionChange} defaultValue={description} name='description' /> 
            </Label>
            <AddStepButton onClick={handleAddStep} type='button'>
                <img src={PlusIconWhite} alt='Add' />
                Add Step
            </AddStepButton>
            ...
            <StepsContainer>
                {steps.map(({content}, index) => (
                    <StepEdit key={index}>
                        <div>{index + 1}</div>
                        <input onChange={(e) => handleChangeStepContent(e, index)} type='text' name='step' value={content} />

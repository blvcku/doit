...
import Person from '../components/friends-person/FriendsPerson';
...
const SearchFriends = ({ searchTerm }) => {
    ...
    const {
        currentUser,
        currentUserData: { friends = [], invites = [], requests = [] } = {},
    } = useAuth();
    const {
        handleNextSlide,
        handlePrevSlide,
    ...
    const { last, setHasMore, currentPage, setCurrentPage, setLoading } =
        useInfiniteScroll();
    const chunkSize = 8;
    ...
    useEffect(() => {
        if (persons) {
            const mappedPersons = persons.map((person) => {
                if (friends.includes(person.uid))
                    return { ...person, status: 'friend' };
                if (invites.includes(person.uid))
                    return { ...person, status: 'invite' };
                if (requests.includes(person.uid))
                    return { ...person, status: 'request' };
                return person;
            });
            const result = [];
                ...
                <FlexContainer currentSlide={currentSlide}>
                    {pages.map((page, pageIndex) => (
                        <GridContainer key={pageIndex}>
                            {page.map(
                                (
                                    { uid, displayName, photoURL, status },
                                    index,
                                ) => {
                                    if (
                                        pages.length === pageIndex + 1 &&
                                        pages[pageIndex].length === index + 1
                                    ) {
                                        return (
                                            <Person
                                                innerRef={last}

...
const FormCreator = () => {
    ...
    const createForm = async (e) => {
        ...
        try {
            ...
            // Creating QRCode
            const { url: QRCode } = await fetch(
                `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${formLink}`,
            );
            // Creating form
            await formRef.set({
            ...
            const moveQuestionFile =
                functions.httpsCallable('moveQuestionFile');
            // Creating questions
            for (const [
                order,
                { title, answers, multipleAnswers, file, inputField },
            ] of questions.entries()) {
                ...
                if (file && file.url) {
                    await storage
                        .ref(`temp/forms/${formID}/${questionID}/file`)
                        .putString(file.url, 'data_url', {
                            customMetadata: { owner: uid },
                        });
                    await moveQuestionFile({
                        formID: formID,
                        questionID: questionID,
                    });
                    url = await storage
                        .ref(`forms/${formID}/${questionID}/file`)
                        .getDownloadURL();
                }
                ...
                const answersSumRef = questionRef
            ...
            <Form onSubmit={createForm}>
                ...
                <CustomNavLink activeClassName="active" to={`${path}/add`}>
                    <svg
                        ...
                        viewBox="0 0 34.355 34.355"
                    >
                        <path
                            id="Path_87"
                            data-name="Path 87"
                            d="M21.178,8.294a4.294,4.294,0,1,1-4.294,4.294,4.307,4.307,0,0,1,4.294-4.294m0,21.472c5.8,0,12.454,2.77,12.883,4.294H8.294c.494-1.546,7.107-4.294,12.883-4.294M21.178,4a8.589,8.589,0,1,0,8.589,8.589A8.587,8.587,0,0,0,21.178,4Zm0,21.472C15.445,25.472,4,28.349,4,34.061v4.294H38.355V34.061C38.355,28.349,26.911,25.472,21.178,25.472Z"

...
import { db, fb, storage, functions } from '../../../services/firebase';
...
const PostCreator = ({
    ...
    const handleChangeFile = (e) => {
        ...
        if (
            ...
            return dispatchError({ type: 'posts/wrong-file-type' });
    ...
    const handleCreatePost = async (e) => {
        ...
        if (!description.trim())
            return dispatchError({ type: 'posts/description-empty' });
        setLoading(true);
        try {
    ...
    const handleUpdatePost = async (e) => {
        ...
        try {
            ...
            if (file && file.url && file.url !== initialFile.url) {
                const movePostFile = functions.httpsCallable('movePostFile');
                await storage
                    .ref(`temp/posts/${postID}/file`)
                    .putString(file.url, 'data_url', {
                        customMetadata: { owner: uid },
                    });
                await movePostFile({ postID: postID });
                const url = await storage
                    .ref(`posts/${postID}/file`)
                    .getDownloadURL();
                await db
                    .collection('posts')
                    .doc(postID)
